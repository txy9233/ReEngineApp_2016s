...
void AppClass::Update(void)
{
	...
	glm::quat rotQuatX = glm::quat(rotX * sin(m_v3Orientation.x));
	glm::quat rotQuatY = glm::quat(rotY * sin(m_v3Orientation.y));
	glm::quat rotQuatZ = glm::quat(rotZ * sin(m_v3Orientation.z));
	glm::quat rotQuat = rotQuatX*rotQuatY*rotQuatZ;

	//linear combination
	//m_mToWorld = rotX * rotY * rotZ;
	
	m_mToWorld = glm::toMat4(rotQuat);
